...
public class SkeletonPlot19 extends java.applet.Applet {
    ...
    Button restartApplet, redoMasterMarks, hintShow, answersShow, masterShow;
    ...
    Color myYellow         = new Color(255,255,204); //15,12
    ...
    public void init() {
        ...
        offscreenImg = createImage(700,700);
...
//        p.setBackground(Color.cyan);
        ...
        Label possibleLabel = new Label("Ring-Growth", Label.CENTER);
        gridbag.setConstraints(possibleLabel, constraints);
        p.add(possibleLabel);
        //
        buildConstraints(constraints, 0, 1, 1, 1, 0, 0);
        Label ringGrowthLabel = new Label("Anomalies", Label.CENTER);
        ...
        Label orLabel = new Label("or", Label.CENTER);
        gridbag.setConstraints(orLabel, constraints);
        p.add(orLabel);
        //
        buildConstraints(constraints, 5, 2, 1, 1, 0, 0);
        Label hideLabel = new Label("Hide", Label.CENTER);
        gridbag.setConstraints(hideLabel, constraints);
        p.add(hideLabel);
        //
/*        buildConstraints(constraints, 5, 1, 1, 1, 0, 0);
        hintShow = new Button("Hint");
...
/*        else if (x>=Math.max((int)Math.round(this.size().width/2-(this.size().width/2-masterLeftX)*graphScale/2),0) &&
        ...
        //else if over entire master plot
        else if (x>=Math.max((int)Math.round(this.size().width/2-(this.size().width/2-masterLeftX)*graphScale/2),0) &&
            x< Math.min(this.size().width,(int)Math.round(this.size().width/2-(this.size().width/2-masterLeftX)*
                        graphScale/2+graphEmptySpace+numberOfMasterLines*lineSpacing*graphScale)) &&
            y>=masterUpperY && y<masterUpperY + masterHeight && showMaster) {
...
//            myFrame.setCursor(Frame.DEFAULT_CURSOR);
    ...
    public boolean mouseDrag(Event evt, int x, int y) {
        ...
        if (x > this.size().width/2-(this.size().width/2-coreMountLeftX)*coreMagLevel &&
            x < this.size().width/2-(this.size().width/2-coreMountLeftX)*coreMagLevel +
               (coreLength+coreMountEdge*2)*coreMagLevel && y>=coreMountUpperY && y<coreMountUpperY+coreMountHeight) {
            coreMountLeftX += (int)Math.round((x-oldX)/coreMagLevel);
            if (coreMountLeftX > this.size().width/2)
                coreMountLeftX = this.size().width/2;
            else if (this.size().width/2-(this.size().width/2-coreMountLeftX)*coreMagLevel +
                    (coreLength+coreMountEdge*2)*coreMagLevel < this.size().width/2)
                coreMountLeftX = (int)(Math.round(this.size().width/2)) - (coreLength+coreMountEdge*2);
            oldX = x;
            redrawCore = true;
        }
        //else if within top third of skeleton plot, move skeleton plot
        else if (x > this.size().width/2-(this.size().width/2-skeletonLeftX)*graphScale/2 &&
                 x < this.size().width/2-(this.size().width/2-skeletonLeftX)*graphScale/2 +
                      graphEmptySpace+numberOfSkeletonLines*lineSpacing*graphScale &&
            y>=skeletonUpperY && y<skeletonUpperY+(int)Math.round(skeletonHeight/3)) {
            skeletonLeftX += (x-oldX);
            oldX = x;
            redrawSkeletonPlot = true;
        }
        //else if within master plot, move master plot
        else if (x>=Math.max((int)Math.round(this.size().width/2-(this.size().width/2-masterLeftX)*graphScale/2),0) &&
            x< Math.min(this.size().width,(int)Math.round(this.size().width/2-(this.size().width/2-masterLeftX)*
                        graphScale/2+graphEmptySpace+numberOfMasterLines*lineSpacing*graphScale)) &&
            y>=masterUpperY && y<masterUpperY+masterHeight) {
            masterLeftX += (x-oldX);
            redrawMasterPlot = true;
            oldX = x;
        }
  ...
  */              ring++;
        ...
        if (redrawSkeletonPlot) {
            ...
            for (int verticalLine = Math.max((int)Math.round((this.size().width/2-(this.size().width/2-skeletonLeftX)*
                ...
                //draw in start red arrow lines
                if (verticalLine == 0) {
                    offscreenG.setColor(Color.red);
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+verticalLine*lineSpacing*graphScale),
                                     skeletonUpperY+(int)Math.round(skeletonHeight/3)+1,
                                    (int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+verticalLine*lineSpacing*graphScale),
                                     skeletonUpperY+skeletonHeight-1);
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+verticalLine*lineSpacing*graphScale),
                                     skeletonUpperY+(int)Math.round(skeletonHeight),
                                    (int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+verticalLine*lineSpacing*graphScale-2*lineSpacing*graphScale),
                                     (int)Math.round(skeletonUpperY+skeletonHeight-2*lineSpacing*graphScale-1));
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+verticalLine*lineSpacing*graphScale-2*lineSpacing*graphScale),
                                     (int)Math.round(skeletonUpperY+skeletonHeight-2*lineSpacing*graphScale-1),
                                    (int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                ...
                //draw in end red arrow lines
                if (verticalLine == (numberOfRings+1)) {
                    offscreenG.setColor(Color.red);
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                    ...
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     ...
                                     skeletonUpperY+(int)Math.round(skeletonHeight),
                                    (int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+(verticalLine-2)*lineSpacing*graphScale+2*lineSpacing*graphScale),
                                     (int)Math.round(skeletonUpperY+skeletonHeight-2*lineSpacing*graphScale-1));
                    offscreenG.drawLine((int)Math.round(this.size().width/2-(this.size().width/2-skeletonLeftX)*
                                     graphScale/2+graphEmptySpace+(verticalLine-2)*lineSpacing*graphScale+2*lineSpacing*graphScale),
                                     (int)Math.round(skeletonUpperY+skeletonHeight-2*lineSpacing*graphScale-1),
        ...
        //draw a border
        offscreenG.setColor(Color.black);
        offscreenG.drawRect(0,0,this.size().width-1, this.size().height-1);
